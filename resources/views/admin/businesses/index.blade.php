                            ...
                            <tr role="row" class="filter">
                                <td>
                                    <div class="input-group width_100">
                                        <input type="text" class="form-control input-inline input-small width_100" name="business_name" placeholder="Name" value="{{session('business_name_filter')}}">
                                        <input type="hidden" class="form-control form-filter input-sm" name="set_session" placeholder="set_session" value="1">
                                    </div>
                                </td>
                                <td>
                                    <div class="input-group width_100">
                                        <input type="text" class="form-control input-inline input-small width_100" name="business_phone" placeholder="Phone" value="{{session('business_phone_filter')}}">
                                    </div>
                                </td>
                                <td>
                                    <div class="input-group width_100">
                                        <select name="business_neighborhood" class="form-control form-filter input-small width_100" id="business-neighborhood">
                                            <option value="" {{ session("business_neighborhood_filter") === "" ? "selected" : ""}}>Select Neighborhood</option>
                                            @foreach($neighborhoods as $k => $neighborhood)
                                                <option value="{{$neighborhood->_id}}" {{ session("business_neighborhood_filter") === $neighborhood->_id ? "selected" : ""}}>{{$neighborhood->neighborhood_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </td>
                                <td>
                                    <div class="input-group width_100">
                                        <input type="text" class="form-control input-inline input-small width_100" name="business_created_by" placeholder="Created By" value="{{session('business_created_by_filter')}}">
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="input-group width_100">
                                        <select name="business_approved" class="form-control form-filter input-small width_100" id="business_approved">
                                            <option value="" {{ session("business_approved_filter") === "" ? "selected" : ""}}>Select Approval Staus</option>
                                            <option value="1" {{ session("business_approved_filter") === "1" ? "selected" : ""}}>Approved</option>
                                            <option value="0" {{ session("business_approved_filter") === "0" ? "selected" : ""}}>Not Approved</option>
                                        </select>
                                    </div>
                                </td>
                                <td>
...
@section("script")
	<script type="text/javascript">
		let url = '{{ url('admin/businesses/get') }}';
		let columns = [
