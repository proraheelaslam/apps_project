...
@section('content')
    <div class="page-content" style="min-height:1240px">
        ...
        <!-- END PAGE BAR -->
        <!-- BEGIN PAGE TITLE-->
        <h3 class="page-title"> {{$user->full_name}}
        </h3>
        <!-- END PAGE TITLE-->
        <!-- END PAGE HEADER-->
        <div class="row">
                    ...
                    <div class="portlet light profile-sidebar-portlet ">
                        ...
                        <div class="profile-userpic">
                            ...
                            @if(\File::exists(public_path('upload/users/'.$user->user_image) ))
                                <strong class="profileImg remove_profile" onclick="removeProfile('{{$user->_id}}')"><img src="{{$user->full_image}}" class="img-responsive" alt=""><i class="fa fa-trash fa-2x" aria-hidden="true"></i></strong>
                                {{--<span title="Delete Profile image" class="remove_profile" onclick="removeProfile('{{$user->_id}}')" style="padding-left: 203px;color: red;"><i class="fa fa-trash fa-2x" aria-hidden="true"></i></span>--}}
                            @else
                                <img src="{{$user->full_image}}" class="img-responsive" alt="">
                            @endif
                            {{--<img src="{{$user->full_image}}" class="img-responsive" alt="">--}}
                        </div>
                        <!-- END SIDEBAR USERPIC -->
                        ...
                        <div class="profile-userbuttons">
                            ...
                            @if($user->status->ustatus_name == 'pending')
                            ...
                            @elseif($user->status->ustatus_name == 'approved')
                            ...
                            @elseif($user->status->ustatus_name == 'deactivate')
                                ...
                                <div class="portlet-body">
                                    <div class="tab-content">
                                        ...
                                        <div class="tab-pane active" id="tab_1_1">
                                            ...
                                            @endif
                                            {!! Form::open(['url'=>'admin/user/profile/update','method' => 'post','class'=> '','data-toggle'=>'validator','role'=>'form']) !!}
                                                <input type="hidden" name="user_id" value="{{$user->_id}}">
                                                <div class="form-group">
                                                    <label class="control-label">First Name</label>
                                                    {!! Form::text('first_name',$user->user_fname,['class'=>'form-control','placeholder'=>'First Name','required'=>'required']) !!}
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label">Last Name</label>
                                                    {!! Form::text('last_name',$user->user_lname,['class'=>'form-control','placeholder'=>'Last name','required'=>'required']) !!}

                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label">Mobile Number</label>
                                                    {!! Form::text('user_phone',$user->user_phone,['class'=>'form-control','placeholder'=>'Phone']) !!}
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label">Email</label>
                                                    {!! Form::text('email',$user->user_email,['class'=>'form-control','placeholder'=>'Email','required'=>'required']) !!}
                                                </div>
                                                <div class="form-group">
                                                    <label class="control-label">Gender</label>
                                                    <select name="gender" class="form-control">
                                                        <option value="Male" {{$user->gender->gender_name =='Male' ? 'selected' : ''}}>Male</option>
                                                        <option value="Female" {{$user->gender->gender_name =='Female' ? 'selected' : ''}}>Female</option>
                                                        <option value="Other" {{$user->gender->gender_name =='Other' ? 'selected' : ''}}>Other</option>
                                                    </select>
                                                </div>
                                        ...
                                        <div class="tab-pane" id="tab_1_3">
                                            @if(!empty($user->user_address_verify_code) || ($user->user_address_verify_code !=0))
                                                <table class="table table-light table-hover">
                                                    <tbody>
                                                    <tr>
                                                        <td> Address Pin Code
                                                            <span class="pull-right">{{$user->user_address_verify_code}}</span>
                                                        </td>
                                                    </tr>
                                                    </tbody>
                                                </table>
                                            @endif
                                                <div class="form-group">
                                                    @if($user->user_is_address_verified == 1)
                                                        @if($user->user_address_type == 'document')
                                                            <img width="200px" height="200px"
                                                                 src="{{$user->user_address_image }}">
                                                            @if(\File::exists(public_path('upload/addresses/'.$user->user_address_document) ))
                                                                <button style="margin-top: 164px;"
                                                                        onclick="removeAddressImage('{{$user->_id}}')" type="button"
                                                                        class="btn btn-circle red btn-sm">Remove image
                                                                </button>
                                                            @endif
                                                            <div class="clearfix"></div><br>
                                                            <button onclick="verifyAddress('{{$user->_id}}')" type="button"
                                        ...
                                        <div class="tab-pane" id="tab_1_4">
                                            @if($user->user_address_latitude == 0 && $user->user_address_longitude == 0)
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <button type="button" onclick="sendMessage('{{$user->_id}}')" class="btn green pull-right btnModal">
                    Send Message
                </button>
...
<script>
    let session = "{{Session::get('verified')}}";
    var user_id = "{{ $user->_id }}";

    let url = '{{ url('admin/chat/get/user/'.$user->_id) }}';
    let columns = [
    ...
    function showLcoation(){
        let user_address_latitude = "{{$user->user_address_latitude}}";
        let user_address_longitude = "{{$user->user_address_longitude}}";
    ...
    function sendMessage(user_id) {
        ...
        $.LoadingOverlay("show");
        $.ajax({
            url: "{{url('admin/user/send_message')}}",
            type: 'post',
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $.LoadingOverlay("show");
                    $.ajax({
                        url: "{{url('admin/user/delete_profile')}}",
                        type: 'post',
        ...
        $.confirm({
            ...
            buttons: {
                confirm: function () {
                    $.LoadingOverlay("show");
                    $.ajax({
                        url: "{{url('admin/user/address/verify')}}",
                        type: 'post',
                        data: {user_id: user_id},
                        success: function (data) {
                            if (data.status == true) {
